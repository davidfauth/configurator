    ...
    public void run() {
        ...
        try {
            ...
            for (CSVRecord record : records) {
				...
				String charCharValueId = record.get("CHAR") + "_" + record.get("CHARVALUE");
				String varTabID = record.get("VARTAB") + "_" + record.get("SEQUENCE");
                Integer sequence = Integer.valueOf(record.get("SEQUENCE"));

                Node variant = db.findNode(Labels.VarTabSeq, "id", id);
                if (variant == null) {
                    variant = db.createNode(Labels.VarTabSeq);
